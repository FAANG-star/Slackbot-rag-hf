...
```python
report_to="trackio"  # Recommended — see Trackio skill for setup
```
...
Use Trackio for real-time metrics visualization and dashboards:
...
See the **hugging-face-trackio** skill for full setup, dashboard configuration, CLI retrieval, and HF Space syncing.
...
Audio tasks require additional packages beyond the basics:
```bash
pip install datasets[audio] torchaudio torchcodec librosa soundfile
apt-get install -y libsndfile1 ffmpeg
```

`torchcodec` and `torchaudio` are required for audio decoding — `datasets` will fail to load audio samples without them.

**Note**: Do NOT use `trust_remote_code=True` with `load_dataset` — it is deprecated and will error on newer versions of `datasets`.

### Audio Preprocessing Pattern

Use `AutoFeatureExtractor` to preprocess audio for the model:
```python
...
dataset = dataset.map(preprocess, remove_columns=["audio", "file"])
```

For classification, use `AutoModelForAudioClassification`. Extract label mappings from the dataset features.
...
4. **Log extensively**: Use Trackio for real-time dashboards
